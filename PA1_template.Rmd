...
bin_width <- round(range(ds_no_na$totalstepsday) / 30)[2]
ggplot(ds_no_na) + 
    geom_histogram(aes(x = totalstepsday),colour = "darkgreen", fill = "white", binwidth = bin_width)  +
    xlab(label = 'Steps') +
    ylab(label = 'Frequency') +
...
```` 
4. The mean of the total number of steps taken per day is `r mean_total_steps_per_day_no_na` and the median is `r median_total_steps_per_day_no_na`. 
